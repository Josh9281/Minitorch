...
class Sigmoid(Function):
    ...
    def forward(ctx: Context, a: Tensor) -> Tensor:
        ...
        return out  # Use the backend's sigmoid method
...
class Exp(Function):
    ...
    def forward(ctx: Context, a: Tensor) -> Tensor:
        ...
        out = a.f.exp_map(a)
        ctx.save_for_backward(out)
        return out  # Use the backend's exp method

    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        """Backward pass for the exponential function."""
        a = ctx.saved_values[0]
        return grad_output.f.mul_zip(a, grad_output)  # Use backend for gradients
...
class Sum(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        a_shape, dim = ctx.saved_tensors  # Get the saved tensor from forward
